...
<?php
    if (!isset($_POST['btnExec'])) {
        ...
        $html = "テキストボックスに値を入力して[送信]ボタンをクリックしてください。
                <form action='sample12-21-1.php' method='POST'>
        ...
        if (strlen($_POST['inputdata']) > 0) {
            print "テキストボックスに入力されたデータは「" . $_POST['inputdata'] . "」です。<br /><br />確定してよろしければ[OK]ボタンをクリックしてください。";
            // 確認用フォームのHTMLを組み立てる
            ...
            // 再入力用フォームのHTMLを組み立てる
            $html = "<form action='sample12-21-1.php' method='POST'>
                        <input size='40' type='text' name='inputdata'>
                        <input type='submit' name='btnExec' value='送信'>
                    </form>";
        }
